...
for pp, beh_path, meg_path in zip_files:
    print(pp)
    ...
    odd_means[pp] = np.mean(np.array([meg[key] for key in keys_odd]), axis=0)
    reg_means[pp] = np.mean(np.array([meg[key] for key in keys_reg]), axis=0)
